<script type="text/javascript">
    ...
    subtitulo: "Gestión de los Video de las Escuelas",
    ...
    fillVideoFacultades:{'id':'', 'titulo':'', 'descripcion':'', 'link':'','fecha':'','activo':'','borrado':'','escuela_id':''},
...
methods: {
    getDescripcionFacultades: function (page) { 
        ...
        var url='videoescuela?page=' + page + '&busca=' + busca; 
    ...
    create: function () {
        ...
        borrardocente: function (videosfacultades) {
            ...
            swal.fire({
                title: '¿Estás seguro?',
                text: "¿Desea eliminar el video de la escuela Seleccionado? -- Nota: este proceso no se podrá revertir.",
                type: 'info',
                ...
                if (result.value) {
                    ...
                    var url = 'videoescuela/' + videosfacultades.id;
    ...
    editbanner: function (videosfacultades) {
        ...
        this.fillVideoFacultades.escuela_id = videosfacultades.idescuela;
    ...
    updateBanner: function (id) {
        ...
        data.append('escuela_id', this.fillVideoFacultades.escuela_id);
        data.append('_method', 'PUT');
        
        
        const config = {headers: {'Content-Type': 'multipart/form-data'}};
        
        var url = "videoescuela/" + id;
        ...
        if (response.data.result == '1') {
            ...
            this.getDescripcionFacultades(this.thispage);
            this.fillLocal= {'id':'', 'titulo':'', 'descripcion':'', 'link':'', 'fecha':'','activo':'','borrado':'','escuela_id':''};
            this.errors = [];
...
bajadocente: function (videosfacultades) {
        ...
        swal.fire({
            title: '¿Estás seguro?',
            text: "Desea desactivar el video de la escuela seleccionada",
            type: 'info',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
        ...
        if (result.value) {
            var url = 'videoescuela/altabaja/' + videosfacultades.id + '/0';
...
altadocente: function (videosfacultades) {
    swal.fire({
        title: '¿Estás seguro?',
        text: "Desea activar el video de la escuela seleccionada",
        type: 'info',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
    ...
    if (result.value) {
        var url = 'videoescuela/altabaja/' + videosfacultades.id + '/1';
        axios.get(url).then(response => { //eliminamos
    if (response.data.result == '1') {
        app.getDescripcionFacultades(app.thispage); //listamos
        toastr.success(response.data.msj); //mostramos mensaje
...
</script><?php /**PATH C:\Users\yuri_\OneDrive\Desktop\webFacultades\resources\views/videoEscuelas/vue.blade.php ENDPATH**/ ?>